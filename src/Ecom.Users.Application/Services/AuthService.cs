...
public class AuthService(
    ...
    [FromKeyedServices(ServiceKeys.QueryRepository)]
        IQueryRepository<RefreshToken, Guid> refreshTokenRepository,
...
) : IAuthService
{
    public async Task<Response<AuthResponseDto>> RegisterAsync(RegisterDto registerDto)
    {
        try
        {
            ...
            {
                var existingUsername = await userRepository.FirstOrDefaultAsync(u =>
            ...
            var defaultRole = await roleRepository.FirstOrDefaultAsync(r =>
                r.Name == AuthConstants.Roles.User
            );
            if (defaultRole == null)
            {
            ...
            var accessToken = jwtService.GenerateAccessToken(
                user,
                permissions.Select(p => p.FullPermission)
            );
            var refreshToken = jwtService.GenerateRefreshToken(user, jwtId);

            // Add entities to context (but don't save yet)
            ...
            when (ex.InnerException?.Message?.Contains("duplicate key") == true)
        {
            logger.LogError(
                ex,
                "Duplicate key error during user registration for email: {Email}",
                registerDto.Email
            );
            return ResponseUtils.Error<AuthResponseDto>(
                409,
    ...
    public async Task<Response<AuthResponseDto>> LoginAsync(LoginDto loginDto)
    {
        try
        {
            ...
            else
            {
                return ResponseUtils.Error<AuthResponseDto>(
                    401,
                    $"This account uses {user.Provider} authentication"
                );
            }
    ...
    public async Task<Response<AuthResponseDto>> HandleOAuthCallbackAsync(
        string provider,
        string code
    )
    {
        ...
        logger.LogInformation(
            "OAuth callback received for provider: {Provider} with code: {Code}",
            provider,
            code
        );
        return ResponseUtils.Error<AuthResponseDto>(
            501,
            "OAuth authentication is not fully implemented in this version"
        );
    }
    ...
    public async Task<Response<bool>> RevokeAllRefreshTokensAsync(Guid userId)
    {
        try
        {
            var refreshTokens = await refreshTokenRepository.GetAllAsync(rt =>
                rt.UserId == userId
                && !rt.IsUsed
                && !rt.IsRevoked
        ...
        catch (Exception ex)
        {
            ...
            return ResponseUtils.Error<bool>(
                500,
                "Token revocation failed: An unexpected error occurred during token revocation"
            );
        }
    }
    ...
    public async Task<Response<bool>> RevokeRefreshTokenAsync(string token)
    {
        try
        {
            ...
            if (refreshToken == null)
            {
                return ResponseUtils.Error<bool>(
                    400,
                    "Token revocation failed: Invalid refresh token"
                );
            }
    ...
    public async Task<Response<bool>> ValidateTokenAsync(string token)
    {
        ...
        catch (Exception ex)
        {
            ...
            return ResponseUtils.Error<bool>(
                500,
                "Token validation failed: An unexpected error occurred during token validation"
            );
        }
    }
    ...
    public async Task<User?> AuthenticateFacebookAsync(string facebookToken)
    {
        await Task.CompletedTask;
        logger.LogInformation(
            "Facebook authentication requested with token: {Token}",
            facebookToken
        );
    ...
    public async Task<Response<AuthResponseDto>> SSOLoginAsync(string provider, string token)
    {
        try
        {
            ...
            switch (provider.ToLower())
            {
                ...
                default:
                    return ResponseUtils.Error<AuthResponseDto>(
                        400,
                        $"SSO Login failed: Unsupported provider '{provider}'"
                    );
            }
            ...
            if (user == null)
            {
                return ResponseUtils.Error<AuthResponseDto>(
                    401,
                    "SSO Login failed: Invalid token or authentication failed"
                );
            }
        ...
        catch (Exception ex)
        {
            ...
            return ResponseUtils.Error<AuthResponseDto>(
                500,
                "SSO Login failed: An unexpected error occurred during authentication"
            );
        }
    }

    public async Task<Response<bool>> UpdateUserAsync(
        Guid currentUserId,
        Guid targetUserId,
    ...
    )
    {
        try
        {
            ...
            if (
                ...
                && updateUserDto.Email != targetUser.Email
            )
            {
                var existingUser = await userRepository.FirstOrDefaultAsync(u =>
                    u.Email == updateUserDto.Email && u.Id != targetUserId
                );
                if (existingUser != null)
                {
            ...
            if (
                ...
                && updateUserDto.UserName != targetUser.UserName
            )
            {
                var existingUsername = await userRepository.FirstOrDefaultAsync(u =>
                    u.UserName == updateUserDto.UserName && u.Id != targetUserId
                );
                if (existingUsername != null)
                {
            ...
            if (!string.IsNullOrEmpty(updateUserDto.NewPassword))
            {
                if (targetUser.Provider != AuthConstants.Providers.Local)
                {
                    return ResponseUtils.Error<bool>(
                        400,
                        "Cannot change password for SSO accounts"
                    );
                }
            ...
            logger.LogInformation(
                "User {TargetUserId} updated by {CurrentUserId}",
                targetUserId,
                currentUserId
            );
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Error updating user {TargetUserId} by {CurrentUserId}",
                targetUserId,
                currentUserId
            );
    ...
    {
        var userRolesAll = await userRepository.GetAllAsync(
            u => u.Id == userId,
            builder: u => u.Include(ur => ur.UserRoles).ThenInclude(ur => ur.Role)
        );
        ...
        var permissionsAll = await roleRepository.GetAllAsync(r => userRoles.Contains(r.Id));
        ...
        var permissions = permissionsAll
            .Entities.SelectMany(r => r.RolePermissions)
